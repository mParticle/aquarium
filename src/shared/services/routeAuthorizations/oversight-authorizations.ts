...
export class OversightAuthorizations extends BaseRoutesAuthorizations {
  ...
  private canViewObservability(): boolean {
    return (
      FeatureFlagsService.isEnabled(FeatureFlag.ObservabilityDetails) &&
      AuthorizationsService.isAuthorized(Operation.OBSERVABILITY_VIEW)
    )
  }

  private canViewObservabilityTraceList(): boolean {
    return (
      FeatureFlagsService.isEnabled(FeatureFlag.ObservabilityM4) &&
      AuthorizationsService.isAuthorized(Operation.OBSERVABILITY_VIEW)
    )
  }
